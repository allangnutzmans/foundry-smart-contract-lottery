...
const RaffleCardRender = () => {
  ...
  const { timeLeft, raffleState, roundId, isWinner, winnerAddress, setWinnerAddress } =
    useRaffleState();
  ...
  const renderRaffleContent = () => {
    ...
    if (!isConnected || balance?.value === BigInt(0)) {
      return <RaffleCardNew balance={balance} entranceFee={entranceFee} roundId={roundId} />;
    }
...
export default RaffleCardRender;
